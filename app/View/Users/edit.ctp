	...
	<div class="row-fluid">
		<?php if (isSet($alert)): ?>
			<div class='alert alert-error'>
				<button type="button" class="close" data-dismiss="alert">&times;</button>
  				<strong>Erro!</strong> Não foi possível atualizar o usuário. Por favor, reveja as informações!
			</div>
		<?php endif; ?>
		<?php echo $this->Form->create('User', array('class' => 'form-horizontal')); ?>
			...
			<div class="control-group <?php if ($this->Form->isFieldError('User.password')) echo 'error' ?>">
				...
				<div class='controls'>
					  <?php echo $this->Form->input('User.password', array('div' => false, 'label' => false, 'value' => ''));
			...
			<div class="control-group">
				<?php echo $this->Form->label('User.type', 'Tipo de usuário: ', array("class" => 'control-label')) ?>
				<div class="controls">
					<label class="radio inline">
					  	<input type="radio" name="data[User][type]" id="UserTypePrincipal" value="0" <?php if ($this->request->data['User']['type'] == 0) echo 'checked' ?> />
					  	Administrador principal
					</label>
					<label class="radio inline">
					  	<input type="radio" name="data[User][type]" id="UserTypeAuxiliary" value="1" <?php if ($this->request->data['User']['type'] == 1) echo 'checked' ?> />
			...
			<div class="form-actions" style="padding-left: 20px">
				<?php echo $this->Form->submit('Atualizar', array('div' => false, 'class' => 'btn btn-primary')); 
