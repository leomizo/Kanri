<?php $this->Paginator->options(array('url' => array('action' => 'get_workplaces', '?' => array('search' => isset($this->request->query['search']) ? $this->request->query['search'] : '')), 'class' => 'pagination-link', 'paginated-content' => '#workplace-content', 'onclick' => 'return Info.handleAsynchronousPagination(this)')); ?>
		...
		<tr class='table-simple-form'>
			...
			<?php echo $this->Form->create('Workplace', array('style' => 'margin-bottom: 0', 'onsubmit' => 'return Info.Workplace.update(this)', 'autocomplete' => 'off')); ?>
				...
				<?php echo $this->Form->label('name', 'Empresa: ', array('div' => false, 'style' => 'font-weight: bold; display: inline'));
				      ...
				      echo $this->Form->input('name', array('value' => $workplace['Workplace']['name'], 'style' => 'display: none', 'div' => false, 'label' => false, 'class' => 'workplace-name-input', 'required' => 'required'));
				...
				<?php echo $this->Form->label('MarketSector.id', 'Segmento: ', array('div' => false, 'style' => 'font-weight: bold; display: inline'));
				      ...
				      echo $this->Form->input('MarketSector.id', array('style' => 'display: none', 'div' => false, 'label' => false, 'class' => 'workplace-market-sector-input', 'id' => false, 'options' => array(), 'onchange' => 'Info.Workplace.editNewMarketSector(this)', 'current' => $workplace['MarketSector']['id'], 'required' => 'required')); 
				...
				<?php echo $this->Form->label('nationality', 'Nacionalidade: ', array('div' => false, 'style' => 'font-weight: bold; display: inline'));
					  ...
					  echo $this->Form->input('nationality', array('value' => $workplace['Workplace']['nationality'], 'style' => 'display: none', 'div' => false, 'label' => false, 'class' => 'workplace-nationality-input', 'required' => 'required')); ?>
				</div>
				<?php echo $this->Form->button('OK', array('class' => 'btn btn-mini btn-primary confirm-btn', 'style' => 'display: none'));
